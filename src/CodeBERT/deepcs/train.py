...
def train(args):
    fh = logging.FileHandler(f"./output/{args.model}/CSN-python/logs.txt")
    # create file handler which logs even debug messages
    logger.addHandler(fh)  # add the handlers to the logger
    timestamp = datetime.now().strftime('%Y%m%d%H%M')
    # tb_writer = SummaryWriter(f"./output/{args.model}/Code2Seq/logs/{timestamp}" ) if args.visual else None
    ...
    from mydata import mydataset_valid, mydataset
    ...
    if args.reload_from > 0:
        ckpt = f'./output/{args.model}/github/models/step{args.reload_from}.h5'
        ...
        for batch in data_loader:
            ...
            if config['fp16']:
                ...
                torch.nn.utils.clip_grad_norm_(
                    amp.master_params(optimizer), 5.0)
            else:
                loss.backward()
            ...
            if itr_global % args.valid_every == 0:
                ...
                valid_result = validate(
                    valid_set, model, 1000, 1, config['sim_measure'])
                logger.info(valid_result)

            if itr_global % args.save_every == 0:
                ckpt_path = f'./output/{args.model}/CSN-python/models/step{itr_global}.h5'
...
def validate(valid_set, model, pool_size, K, sim_measure):
    ...
    data_loader = torch.utils.data.DataLoader(dataset=valid_set, batch_size=100,
                                              shuffle=True, drop_last=True, num_workers=1)
    r1s, r5s, r10s, mrrs = [], [], [], []
    code_reprs, desc_reprs = [], []
    n_processed = 0
    for batch in tqdm(data_loader):
        if len(batch) == 8:  # names, name_len, apis, api_len, toks, tok_len, descs, desc_len, bad_descs, bad_desc_len
...
def parse_args():
    ...
    parser.add_argument('--model', type=str,
                        default='JointEmbeder', help='model name')
    parser.add_argument('--dataset', type=str, default='github',
                        help='name of dataset.java, python')
    parser.add_argument('--reload_from', type=int,
                        default=-1, help='epoch to reload from')

    parser.add_argument('-g', '--gpu_id', type=int, default=0, help='GPU ID')
    parser.add_argument('-v', "--visual", action="store_true",
                        default=False, help="Visualize training status in tensorboard")
    parser.add_argument('--automl', action='store_true',
                        default=False, help='use automl')
    # Training Arguments
    parser.add_argument('--log_every', type=int, default=100,
                        help='interval to log autoencoder training results')
    parser.add_argument('--valid_every', type=int,
                        default=5000, help='interval to validation')
    ...
    parser.add_argument('--sim_measure', type=str, default='cos',
                        help='similarity measure for training')
...
if __name__ == '__main__':
    ...
    os.makedirs(
        f'./output/{args.model}/{args.dataset}/tmp_results', exist_ok=True)
