...
# add parent folder to path so as to import common modules
...
class JointEmbeder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.name_encoder = SeqEncoder(
            config['n_words'], config['emb_size'], config['lstm_dims'])
        self.api_encoder = SeqEncoder(
            config['n_words'], config['emb_size'], config['lstm_dims'])
        self.tok_encoder = BOWEncoder(
            config['n_words'], config['emb_size'], config['n_hidden'])
        self.desc_encoder = SeqEncoder(
            config['n_words'], config['emb_size'], config['lstm_dims'])
    ...
    def code_encoding(self, name, name_len, tokens, tok_len):
        ...
        code_repr = self.fuse3(torch.tanh(
            self.w_name(name_repr)+self.w_tok(tok_repr)))
        return code_repr
    ...
    def similarity(self, code_vec, desc_vec):
        ...
        https://arxiv.org/pdf/1508.01585.pdf
        """
        assert self.conf['sim_measure'] in ['cos', 'poly', 'euc',
                                            'sigmoid', 'gesd', 'aesd'], "invalid similarity measure"
        if self.conf['sim_measure'] == 'cos':
            ...
            # or torch.norm(code_vec-desc_vec,2)
