...
class CodeSearchDataset(data.Dataset):
    ...
    def __getitem__(self, offset):
        len1, pos = self.idx_names[offset]['length'], self.idx_names[offset]['pos']
        name_len = min(int(len1), self.max_name_len)
        name = self.names[pos: pos+name_len]
        name = self.pad_seq(name, self.max_name_len)

        # len1, pos = self.idx_apis[offset]['length'], self.idx_apis[offset]['pos']
        # api_len = min(int(len1), self.max_api_len)
        # apiseq = self.apis[pos:pos+api_len]
        # apiseq = self.pad_seq(apiseq, self.max_api_len)

        len1, pos = self.idx_tokens[offset]['length'], self.idx_tokens[offset]['pos']
        tok_len = min(int(len1), self.max_tok_len)
...
def save_vecs(vecs, fout):
    ...
    ds = fvec.create_carray(fvec.root, 'vecs', atom,
                            vecs.shape, filters=filters)
    ds[:] = vecs
    print('done')
    fvec.close()
...
if __name__ == '__main__':
    ...
    train_set = CodeSearchDataset(
        input_dir, 'train.name.h5', 6, 'train.tokens.h5', 30, 'train.desc.h5', 30)
    train_data_loader = torch.utils.data.DataLoader(
        dataset=train_set, batch_size=1, shuffle=False, num_workers=1)
    use_set = CodeSearchDataset(
        input_dir, 'use.name.h5', 6, 'use.apiseq.h5', 20, 'use.tokens.h5', 30)
    use_data_loader = torch.utils.data.DataLoader(
        dataset=use_set, batch_size=1, shuffle=False, num_workers=1)
