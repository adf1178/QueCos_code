    ...
    def train(self, start_epoch, end_epoch, pretrain_critic, start_time=None):
        ...
        for epoch in range(start_epoch, end_epoch + 1):
            ...
            if self.optim.lr < lib.Constants.MIN_LR:
                print("(Actor) Early stop when learning rate is lower than %s" % (
                    str(lib.Constants.MIN_LR)))
                break

            if pretrain_critic:
                print("Pretrain critic...")

            train_reward, critic_loss = self.train_epoch(
                epoch, pretrain_critic, False)
            ...
            print("Validation sentence reward: %.2f" %
                  (valid_sent_reward * 100))
            ...
            if self.opt.has_baseline:
                checkpoint.update(
                    {"critic": self.critic, "critic_optim": self.critic_optim})
            ...
            model_name = os.path.join(
                save_dir, "%s_%s.pt" % (save_name, epoch))
    ...
    def train_epoch(self, epoch, pretrain_critic, no_update):
        ...
        for i in range(len(self.train_data)):
            ...
            rewards, samples = self.sent_reward_func(
                ...
                codes=batch[6].t().tolist(),
                names=batch[8].t().tolist(),
                raw_codes=batch[10],
            ...
            rewards = Variable(torch.FloatTensor(
                [rewards] * samples.size(0)).contiguous())
            if self.opt.cuda:
            ...
            if self.opt.has_baseline:
                # Update critic.
                baselines = self.critic(
                    (batch[0], batch[1], samples, batch[3]), eval=False, regression=True)
            ...
            if not pretrain_critic and not no_update:
                ...
                actor_loss = self.actor.backward(
                    outputs, samples, actor_weights, 1, self.actor_loss_func)
                self.optim.step()
            else:
                actor_loss = 0
            ...
            print("iteration: %s, loss: %s " %
                  (self.opt.iteration, actor_loss))
            print("iteration: %s, reward: %s " %
                  (self.opt.iteration, (report_reward / report_sents) * 100))
