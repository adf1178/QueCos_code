...
from ...lib.model.transformer.Batch import create_masks
from ...lib.model.transformer.Models import TransformerWapper, fake_opt
from ... import lib
import sys
...
class Trainer(object):
    ...
    def train(self, start_epoch, end_epoch, start_time=None):
        ...
        for epoch in range(start_epoch, end_epoch + 1):
            ...
            if self.optim.lr < lib.Constants.MIN_LR:
                print("Early stop when learning rate is lower than %s" %
            ...
            if self.DEV is not None:
                dev_loss, dev_sent_reward, dev_corpus_reward = self.evaluator.eval(
                    self.DEV)
            ...
            save_name = "%smodel_xent%s" % (
                self.opt.data_name, self.opt.show_str)
    ...
    def train_epoch(self, epoch):
        ...
        for i in range(len(self.train_data)):
            ...
            if isinstance(self.model, TransformerWapper):
                src = batch[0][0].t()
                trg = batch[2].t()
                targets = batch[2]
                trg_input = trg[:, :-1]
                src_mask, trg_mask = create_masks(src, trg_input, opt)
                src_mask, trg_mask = src_mask.cuda(), trg_mask.cuda()
                preds = self.model.super_forward(src, trg_input, src_mask, trg_mask)
                ...
                loss = self.model.backward(
                    outputs, targets, weights, num_words, self.loss_func)
                self.opt.iteration += 1
                if self.opt.iteration % 10 == 0:
                    print("iteration: %s, loss: %s " % (self.opt.iteration, loss))
