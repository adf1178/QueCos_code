...
for data_name in ["DEV", "EVAL"]:
    with open(os.path.join(os.path.dirname(__file__), "./from_codenn/%s_ref.txt" % data_name), 'r') as f:
        goldMap = defaultdict(list)
        for line in f.readlines():
            rid, sent = line.split('\t')
            if type(sent) is not str:
                sent = sent.encode('utf-8')  # avoid unicode
...
def clean_up_sentence_string(sent, dict):
    ...
    if type(sent_string) is not str:
        sent_string = sent_string.encode('utf-8')
...
def single_sentence_bleu(pair, tgt_dict):
    ...
    if len_pred == 0:
        ...
        # codenn_bleu
        (goldMap, predictionMap) = lib.codenn_bleu.computeMapsFromPairList(
            [(0, pred_string)], [(0, gold_string)])
...
def sentence_bleu(preds, golds, tgt_dict):
    ...
    results = list(map(single_sentence_bleu, zip(
        preds, golds), [tgt_dict] * len(preds)))
...
def sentence_bleu_codenn(preds, indices, data_name, tgt_dict):
    ...
    for pred in preds:
        cleaned_pred, cleaned_pred_string = clean_up_sentence_string(
            pred, tgt_dict)
        cleaned_preds.append(cleaned_pred)
        cleaned_pred_strings.append(cleaned_pred_string)
    cleaned_indices = [idx[1] for idx in indices]

    predictionMap = {idx: [pred_string] for idx, pred_string in zip(
        cleaned_indices, cleaned_pred_strings)}
    scores = lib.codenn_bleu.bleuListFromMaps(
        codenn_goldMaps[0] if data_name == "DEV" else codenn_goldMaps[1], predictionMap, cleaned_indices)
