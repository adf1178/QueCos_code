...
class mydataset(data.Dataset):
    def __init__(self, max_name_len, max_tok_len, max_desc_len, max_api_len):
        ...
        self.trainset = np.load(os.path.join(os.path.dirname(
            __file__), "../../data-staqc/train.npy"), allow_pickle=True).item()
        # self.names = self.trainset["name_array"]
        # self.idx_names = self.trainset["name_lenpos"]

        self.apis = np.zeros((30)).astype('int64')

        self.desc = self.trainset["query_array"]
        self.idx_descri = self.trainset["query_array"]
...
class mydataset_valid(data.Dataset):
    def __init__(self, max_name_len, max_tok_len, max_desc_len, max_api_len):
        ...
        self.desc = self.validset["query_array"]
        self.idx_descri = self.validset["query_lenpos"]
...
class mydataset_test(data.Dataset):
    def __init__(self, max_name_len, max_tok_len, max_desc_len, max_api_len):
        ...
        self.testset = np.load(os.path.join(os.path.dirname(
            __file__), "../../data-java/test.npy"), allow_pickle=True).item()
    ...
    def __getitem__(self, offset):
        ...
        return token, token_len, good_desc, good_desc_len
...
class mydataset_wordnet(data.Dataset):
    def __init__(self, max_name_len, max_tok_len, max_desc_len, max_api_len):
        ...
        desc_dict = json.loads(
            open('/data/home/zhnong/ke/data-java/descri.json').read())
                    ...
                    for l in syn.lemmas():
                        if(count < 3):
                            if l.name() not in synonyms:
                                ...
                                count += 1
            synonyms = (list(i) + [desc_dict[j] if j in desc_dict else desc_dict['<nofd>']
