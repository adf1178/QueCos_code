...
from mydata import mydataset, mydataset_valid
import math
import logging
...
def train(args):
    fh = logging.FileHandler(f"./output/python/{args.dataset}/logs2.txt")
                                      # create file handler which logs even debug messages
    logger.addHandler(fh)# add the handlers to the logger
    timestamp = datetime.now().strftime('%Y%m%d%H%M')
    tb_writer = SummaryWriter(f"./output/python/{args.dataset}/logs/{timestamp}" ) if args.visual else None
    ...
    def save_model(model, epoch):
        torch.save(model.state_dict(), f'./output/python/staqc/models/{epoch}.h5')
        # torch.save(model, f'./output/python/staqc/models/{epoch}.h5')

    def load_model(model, epoch, to_device):
        assert os.path.exists(f'./output/python/staqc/models/{epoch}.h5'), f'Weights at epoch {epoch} not found'
        model.load_state_dict(torch.load(f'./output/python/staqc/models/{epoch}.h5', map_location=to_device))
    ...
    # train_set = eval(config['dataset_name'])(data_path, config['train_name'], config['name_len'],  config['api_len'], config['train_tokens'], config['tokens_len'], config['train_desc'], config['desc_len'])
    # valid_set=eval(config['dataset_name'])(data_path, config['valid_name'], config['name_len'],  config['api_len'], config['valid_tokens'], config['tokens_len'], config['valid_desc'], config['desc_len'])
    train_set = mydataset(6,50,30,30)
    ...
    for epoch in range(int(args.reload_from/n_iters)+1, config['nb_epoch']+1):
        ...
        for batch in data_loader:
            ...
            if itr_global % args.valid_every == 0:
                ...
                acc1, mrr, map1, ndcg = validate(valid_set, model, 1000, 1, config['sim_measure'])
...
if __name__ == '__main__':
    ...
    os.makedirs(f'./output/python/{args.dataset}/models', exist_ok=True)
    os.makedirs(f'./output/python/{args.dataset}/tmp_results', exist_ok=True)
